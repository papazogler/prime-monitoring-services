...
exports.show = function (req, res) {
  ...
  var q = 'select s.CCCCUSTSHIP, s.name ShipName, v.mtrl instrumentId, v.name name, v.sncode sncode, f.date01 certDate, case  f.CCCSTATUS when 1 then \'Defective\' when 2 then \'Beyond Repair\' when 3 then \'Operative\' when 4 then \'History\' when 5 then \'In Service\' end status from (select m.mtrl, d.sncode, max(f1.findoc) findoc, m.name from mtrdoc d inner join FINDOC f1 on d.findoc = f1.findoc inner join mtrl m on m.mtrl= d.mtrl where d.sncode is not null group by m.mtrl, d.sncode, m.name) v inner join FINDOC f on f.findoc = v.findoc inner join CCCCUSTSHIP s on f.CCCCUSTSHIP = s.CCCCUSTSHIP and f.trdr = s.trdr where s.trdr = ' +id + ' and s.CCCCUSTSHIP=' + shipId + ' and f.CCCSTATUS in (1, 2, 3, 5) order by s.name';
